...
* returning a query result, or 
* returning an event to be published.
...
Using this small library (<64 kByte), you are not required to create port interfaces to change technologies anymore. And when you follow the design principles, you will end up with pure domain code. That domain code doesn't communicate with technical infrastructure, not even through interfaces. That's why you can test the domain code without mocking anything.
